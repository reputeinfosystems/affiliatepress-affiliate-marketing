...
if( !class_exists('affiliatepress_woocommerce') ){
    ...
    class affiliatepress_woocommerce Extends AffiliatePress_Core{
        ...
        */
        function __construct(){            
            ...
            if($this->affiliatepress_woocommerce_commission_add() && $affiliatepress_is_woocommerce_active){
                ...
                add_action( 'woocommerce_order_status_changed', array($this,'affiliatepress_reject_commission_on_refund'), 10, 3 );

                /* Update the status of the commission to "rejected" when the originating order is cancelled or failed payment */
                add_action( 'woocommerce_order_status_changed', array($this,'affiliatepress_reject_commission_on_order_fail'), 10, 3 );

                /* Update the status of the commission to "rejected" when the originating order is trashed */
                add_action( 'wc-on-hold_to_trash', array($this,'affiliatepress_reject_commission_on_trash'), 10 );
                add_action( 'wc-processing_to_trash', array($this,'affiliatepress_reject_commission_on_trash'), 10 );
                add_action( 'wc-completed_to_trash', array($this,'affiliatepress_reject_commission_on_trash'), 10 );
        ...
        */
        ...
        /**
         * Function for Update the status of the commission to "rejected" when the originating order is trashed
         *
         * @param int $affiliatepress_order_id
         *
         */
        function affiliatepress_reject_commission_on_trash( $affiliatepress_order_id ) {
            global $AffiliatePress, $affiliatepress_commission_debug_log_id,$affiliatepress_tbl_ap_affiliate_commissions;
            if ( is_a( $affiliatepress_order_id, 'WP_Post' ) ){
                $affiliatepress_order_id = $affiliatepress_order_id->ID;
            }
            if ( get_post_type( $affiliatepress_order_id ) != 'shop_order' ){
                return;
            }
            $affiliatepress_all_commission_data = $AffiliatePress->affiliatepress_get_all_commission_by_order_and_source($affiliatepress_order_id, $this->affiliatepress_integration_slug);    
                ...
                foreach($affiliatepress_all_commission_data as $affiliatepress_commission_data){
                    ...
                    if(!empty($affiliatepress_commission_data)){
                        ...
                        if($affiliatepress_ap_commission_id != 0){
                            ...
                            $affiliatepress_debug_log_msg = sprintf( 'WOO: Commission #%s successfully marked as rejected, after order #%s move to trash.', $affiliatepress_ap_commission_id, $affiliatepress_order_id );
        ...
        /**
         * Function for reject commission 
         *
         * @param  int $affiliatepress_order_id
         * @param  string $affiliatepress_status_from
         * @param  string $affiliatepress_status_to
         * @return void
         */
        function affiliatepress_reject_commission_on_order_fail($affiliatepress_order_id, $affiliatepress_status_from, $affiliatepress_status_to){
            global $AffiliatePress, $affiliatepress_commission_debug_log_id,$affiliatepress_tbl_ap_affiliate_commissions;
            if ( $affiliatepress_status_to != 'failed' && $affiliatepress_status_to != 'cancelled' ){
                return;
            }
            $affiliatepress_all_commission_data = $AffiliatePress->affiliatepress_get_all_commission_by_order_and_source($affiliatepress_order_id, $this->affiliatepress_integration_slug);
            ...
            if(!empty($affiliatepress_all_commission_data)){
                ...
                foreach($affiliatepress_all_commission_data as $affiliatepress_commission_data){
                    ...
                    if(!empty($affiliatepress_commission_data)){
                        ...
                        if($affiliatepress_ap_commission_status == 4){
                            $affiliatepress_debug_log_msg = sprintf( 'Commission #%s could not be rejected because it was already paid.', $affiliatepress_ap_commission_id );
                            do_action('affiliatepress_commission_debug_log_entry', 'commission_tracking_debug_logs', $this->affiliatepress_integration_slug.' : Commission Not Reject ', 'affiliatepress_'.$this->affiliatepress_integration_slug.'_commission_tracking', $affiliatepress_debug_log_msg, $affiliatepress_commission_debug_log_id);
                            continue;
                        }
                        if($affiliatepress_ap_commission_id != 0){
                            $affiliatepress_commission_data = array(
         ...
         * @return void
        */
        function affiliatepress_reject_commission_on_refund($affiliatepress_order_id, $affiliatepress_status_from, $affiliatepress_status_to){            
            global $AffiliatePress,$affiliatepress_tbl_ap_affiliate_commissions,$affiliatepress_commission_debug_log_id;
            $affiliatepress_reject_commission_on_refund = $AffiliatePress->affiliatepress_get_settings('woocommerce_reject_commission_on_refund', 'integrations_settings');
            if ( $affiliatepress_status_to != 'refunded' ) {
                return;
            }
